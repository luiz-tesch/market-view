...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo h1 { font-size: 1.8rem; font-weight: 800; letter-spacing: -0.02em; color: #fff; }
  .logo span {
  ...
  .dry-badge {
    ...
    padding: 0.3rem 0.8rem; border-radius: 2px; letter-spacing: 0.1em;
  ...
  .stat-label {
    ...
    text-transform: uppercase; margin-bottom: 0.5rem; font-family: 'Space Mono', monospace;
  }
  ...
  .legend-item { display: flex; flex-direction: column; gap: 0.3rem; }
  .legend-field {
    font-family: 'Space Mono', monospace; font-size: 0.7rem;
    color: var(--accent3); font-weight: 700; letter-spacing: 0.05em;
  }
  .legend-desc { font-size: 0.75rem; color: var(--muted); line-height: 1.4; }
  ...
  .btn-run {
    ...
    border: none; padding: 0.75rem 2rem; cursor: pointer; font-weight: 700; transition: all 0.15s;
  }
  ...
  .limit-control label { letter-spacing: 0.05em; }
  .limit-control select {
    background: var(--surface); border: 1px solid var(--border); color: var(--text);
    font-family: 'Space Mono', monospace; font-size: 0.7rem; padding: 0.4rem 0.6rem;
    cursor: pointer; outline: none;
  }
  .limit-control select:focus { border-color: var(--accent3); }

  .status-text { font-family: 'Space Mono', monospace; font-size: 0.7rem; color: var(--muted); letter-spacing: 0.05em; }
  ...
  .markets-grid { display: grid; gap: 0.75rem; }
  .market-card {
    background: var(--surface); border: 1px solid var(--border);
    padding: 1.25rem 1.5rem;
    display: grid; grid-template-columns: 1fr auto auto auto auto auto;
    gap: 1.5rem; align-items: center; transition: border-color 0.2s;
  ...
  .market-question { font-size: 0.9rem; font-weight: 600; color: #fff; line-height: 1.3; }
  .market-reasoning {
    font-size: 0.72rem; color: var(--muted); margin-top: 0.4rem;
    line-height: 1.5; font-family: 'Space Mono', monospace;
  }
  .market-factors { margin-top: 0.35rem; display: flex; gap: 0.4rem; flex-wrap: wrap; }
  .factor-tag {
    font-family: 'Space Mono', monospace; font-size: 0.58rem;
    background: rgba(51,153,255,0.08); border: 1px solid rgba(51,153,255,0.15);
    color: #6699cc; padding: 0.15rem 0.45rem; border-radius: 2px;
  }

  .metric { text-align: center; min-width: 70px; }
  .metric-label {
    font-size: 0.58rem; letter-spacing: 0.12em; color: var(--muted);
    text-transform: uppercase; font-family: 'Space Mono', monospace; margin-bottom: 0.3rem;
  }
  ...
  .confidence-badge {
    ...
    padding: 0.2rem 0.5rem; border-radius: 2px; text-transform: uppercase; display: block;
  }
  ...
  .action-badge {
    ...
    letter-spacing: 0.1em; padding: 0.4rem 0.9rem; border-radius: 2px;
    text-transform: uppercase; min-width: 65px; text-align: center; display: block;
  }
...
<div class="container">
  ...
  <div class="legend">
    ...
    <div class="legend-item">
      ...
      <div class="legend-desc">BUY / SELL if edge exceeds the MIN_EDGE threshold. SKIP if the opportunity is too small to justify the risk.</div>
    </div>
    <div class="legend-item">
      <div class="legend-field">Key Factors</div>
      <div class="legend-desc">Main reasons behind LLaMA's probability estimate for this market.</div>
    </div>
  </div>
...
async function runAnalysis() {
  ...
  document.getElementById('markets-grid').innerHTML = `
    <div class="empty-state">
      ...
      <p style="margin-top:1rem">Fetching ${limit} random markets and running AI analysis...<br><small style="opacity:0.5">This may take ~${Math.round(limit * 4)}s</small></p>
    </div>`;

  try {
    const resp = await fetch('/api/analyze?limit=' + limit);
    ...
    document.getElementById('last-update').textContent =
      'Last run: ' + new Date().toLocaleTimeString('en-US') + ' · ' + data.results.length + ' markets';
    status.innerHTML = `✓ Done — ${data.results.length} markets analyzed`;
    status.className = 'status-text';
  } catch(e) {
...
function renderResults(results) {
  ...
  if (!results.length) {
    grid.innerHTML = '<div class="empty-state"><p>No markets returned.</p></div>';
    return;
  }
  ...
  grid.innerHTML = results.map((r, i) => {
    ...
    <div class="market-card ${cardClass}" style="animation-delay:${delay}s">
      ...
      <div class="metric">
        <div class="metric-label">LLaMA Est.</div>
...
def api_analyze():
    ...
    try:
        ...
        # Fetch a large pool then sample randomly for variety
        ...
        for market in sample:
            ...
            try:
                ...
                results.append({
                    ...
                    "reasoning": f"Error: {str(e)}", "key_factors": [],
                })
...
def request_limit():
    ...
    return request.args.get("limit", 10)
